    ...
    if (trimmedLine.startsWith('- ') || trimmedLine.startsWith('* ')) {
      ...
      elements.push(
        <ul key={`ul-${key++}`} className="mb-2 last:mb-0 ml-4 list-disc">
          <li className="mb-1">{parseInlineMarkdown(trimmedLine.substring(2))}</li>
        </ul>
      );
    }
...
function parseInlineMarkdown(text: string): (string | JSX.Element)[] {
  ...
  while (remaining.length > 0) {
    ...
    if (boldMatch) {
      ...
      elements.push(<strong key={`b-${key++}`}>{parseTextWithLineBreaks(boldMatch[2], key)}</strong>);
    ...
    if (italicMatch && !remaining.match(/^\*\*/)) { // Make sure it's not part of bold
      ...
      elements.push(<em key={`i-${key++}`}>{parseTextWithLineBreaks(italicMatch[2], key)}</em>);
    ...
    if (codeMatch) {
      ...
      elements.push(
        <code key={`c-${key++}`} className="bg-muted px-1 py-0.5 rounded text-xs font-mono">
          {codeMatch[2]}
        </code>
      );
    ...
    if (linkMatch) {
      ...
      elements.push(
        <a key={`l-${key++}`} href={linkMatch[3]} target="_blank" rel="noopener,noreferrer" 
      ...
      <div className="p-3 flex-1 flex flex-col">
        ...
        {tags?.length ? (
          <div className="responsive-tag mt-auto pt-3 flex flex-wrap gap-1.5">
            {tags.map((t: string) => (
              <span key={t} className="select-none hover:bg-[hsl(var(--muted))] hover:text-[hsl(var(--primary))] inline-flex items-center rounded-full border border-border bg-secondary/30 px-3 py-1 text-[12px] transition-colors duration-500 ease-out">
                {t}
              </span>
            ))}
          </div>
  ...
  const activeItems = useMemo(() => {
    ...
    if (query) {
      ...
      arr = arr.filter(p =>
        ...
        norm((p as any).summary ?? (p as any).description ?? "").includes(q) ||
    ...
    if (tag) {
      const t = norm(tag);
      arr = arr.filter(p => (p.tags || []).some(x => norm(x) === t || norm(x).includes(t)));
    }

    if (group) {
      const g = norm(group);
      arr = arr.filter(p =>
        Array.isArray(p.group) ? p.group.some(x => norm(x) === g) : norm(p.group || "") === g
      );
    }
