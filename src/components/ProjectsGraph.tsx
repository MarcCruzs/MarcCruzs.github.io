...
function seedPositions(nodes: any[], outerR: number, innerR: number) {
  ...
  prj.forEach((n: any, i: number) => {
    ...
    n.x = Math.cos(t) * outerR;
    n.y = Math.sin(t) * outerR;
  });
  oth.forEach((n: any, i: number) => {
    const t = (i / Math.max(1, oth.length)) * twoPi;
    n.x = Math.cos(t) * innerR;
    n.y = Math.sin(t) * innerR;
  });
}
...
function buildGraph(items: Project[], mode: "tags" | "groups"): GData {
  ...
  const links: GLink[] = [];
  items.forEach((p) =>
    nodes.push({ id: p.id, name: p.title, type: "project", url: p.url })
  );
  ...
  if (mode === "tags") {
    ...
    [...tagSet].forEach((t) =>
      nodes.push({ id: "tag:" + t, name: t, type: "tag" })
    );
    items.forEach((p) =>
      p.tags.forEach((t) => links.push({ source: p.id, target: "tag:" + t }))
    );
  } else {
    const arr = (g?: string | string[]) =>
      g ? (Array.isArray(g) ? g : [g]) : [];
    const groupSet = new Set<string>();
    items.forEach((p) => arr(p.group).forEach((g) => groupSet.add(g)));
    [...groupSet].forEach((g) =>
      nodes.push({ id: "group:" + g, name: g, type: "group" })
    );
    items.forEach((p) =>
      arr(p.group).forEach((g) =>
        links.push({ source: p.id, target: "group:" + g })
      )
    );
...
export default function ProjectsGraph({
  ...
  useEffect(() => {
    ...
    fgRef.current.d3Force(
      "radial",
      d3
        .forceRadial((n: any) => (n.type === "group" ? inner : outer), 0, 0)
        .strength(0.04)
    );
  ...
  return (
    <div className={className}>
      ...
      <div
        ref={wrapRef}
        className="relative h-[360px] sm:h-[470px] lg:h-[470px] w-full rounded-md overflow-hidden border border-foreground/30"
      >
        {size.w > 0 && size.h > 0 && (
          <ForceGraph2D
            ref={fgRef}
            ...
            nodeCanvasObject={(node: any, ctx, globalScale) => {
              ...
              const tintedIcon = getTintedIcon(
                key,
                url,
                colors.nodeStroke,
              ...
              if (
                tintedIcon &&
                ((tintedIcon as HTMLImageElement).complete ||
                  (tintedIcon as HTMLCanvasElement).width > 0)
              ) {
                ctx.save();
                ctx.globalAlpha = alpha;
                ...
                const originalIcon = getIcon(key, url, () =>
                  (fgRef.current as any)?.refresh?.()
                );
