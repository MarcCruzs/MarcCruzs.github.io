...
export type PinnedSpec = {
  // preferred size (per breakpoint or global)
  w?: number; h?: number; x?: number; y?: number;
  bp?: Partial<Record<"lg"|"md"|"sm"|"xs"|"xxs", { w?: number; h?: number; x?: number; y?: number }>>;
  weight?: number; // boosts score when sorting (e.g., hero = 100)
};

export type LayoutInputs = {
  // full project objects so we can score them
  ...
  // current filter/sort intent:
  filter?: { tag?: string; group?: string; query?: string };
  // optional custom scoring hook
...
function sizeFor(id: string, pinned: LayoutInputs["pinned"], bp: keyof Layouts, defaults: { w: number; h: number }, maxCols: number) {
    ...
    // inside sizeFor(...) or right after you compute sizes per breakpoint:
    const isSmall = bp === "sm" || bp === "xs" || bp === "xxs";
    // force full width on small breakpoints
...
export function buildLayoutsSmart({
  ...
  for (const bp of bps) {
    ...
    // Partition: explicit XY pins vs items to auto-pack
    ...
    // Build column heights starting from pinned tiles
    ...
    // Sort remaining items by score (desc), then stable by id
    ...
    // Greedy pack: place each item at the column position that yields smallest top
